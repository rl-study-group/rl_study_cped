...
class FLOWTrainer(TorchTrainer):
    def __init__(
        ...
        self.margin_prob_thresh=margin_prob_thresh-3
    ...
    # Random weight term for interpolation between real and fake data
        alpha = torch.randn((real.size(0), 1)).to(device=torch.device('cuda' if torch.cuda.is_available() else 'cpu'))

    # Get random interpolation between real and fake data
        interpolates = (alpha * real + ((1 - alpha) * fake)).requires_grad_(True)

        model_interpolates,model_logit = model(interpolates)
        grad_outputs = torch.ones(model_interpolates.size(), requires_grad=False).to(device=torch.device('cuda' if torch.cuda.is_available() else 'cpu'))
    ...
    def train_from_torch(self, batch):
        ...
        if self._n_train_steps_total>850000:
           self.log_alpha=0.5
        elif self._n_train_steps_total>700000 and self._n_train_steps_total<=850000:
           self.log_alpha=1
        elif self._n_train_steps_total>500000 and self._n_train_steps_total<=700000:
           self.log_alpha=1
        ...
        real_data=real_data.cpu()+np.random.normal(0.0, 1.0, size=real_data.size())
        real_data=real_data.to(torch.float32)
        real_data=real_data.to(device=torch.device('cuda' if torch.cuda.is_available() else 'cpu'))
        ...
        if self._n_train_steps_total%5==0  and self._n_train_steps_total<500000:
       ...
       ### update the flow(G) network
        if self._n_train_steps_total%1==0 and self._n_train_steps_total<500000:
           if not torch.isnan(g_loss).any() and not torch.isinf(g_loss).any():
                  self.flow_optimizer.zero_grad()
                  g_loss.backward()
                  self.flow_optimizer.step()
        
        #print(self.flow.scale_factor.grad)
        ...
        """
        if self._n_train_steps_total >= 30000:
        ...
        if self._n_train_steps_total % self.policy_and_target_update_period == 0:
            ...
            #print(torch.max(fakez),torch.max(jacc))
            neg_li = -self.compute_log_density_x(fakez, jacc, self.prior)
            self.margin_prob_thresh=log_li.detach()
            #self.margin_prob_thresh=0
            
            
            if self._n_train_steps_total >= 50000:
             policy_loss = - q_output.mean()+self.log_alpha *(neg_li - self.margin_prob_thresh).mean()
            else:
             policy_loss = self.log_alpha *(neg_li - self.margin_prob_thresh).mean()
